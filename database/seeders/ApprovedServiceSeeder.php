...
class ApprovedServiceSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $unique = $dummyApprovedServices->unique('transaction_code');

        try {
            DB::beginTransaction();
            foreach ($unique->values()->all() as $key => $value) {
                $randomTechnician = rand(17, 32);
                $randomCustomer = rand(9, 24);

                $newapproveservice = new ApprovedService;
                $newapproveservice->technician_id = $randomTechnician;
                $newapproveservice->customer_id = $randomCustomer;
                $newapproveservice->transaction_code = $value['transaction_code'];
                $newapproveservice->summary_problem = "";
                $newapproveservice->save();
            }
            DB::commit();
        } catch (\Throwable $th) {
            DB::rollBack();
